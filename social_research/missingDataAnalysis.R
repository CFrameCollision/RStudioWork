...
fit_imp <- with(
  imp,
  lm(
    CV_HIGHEST_DEGREE_EVER_EDT_2017 ~ KEY_RACE_ETHNICITY_1997 +
      CV_HGC_RES_MOM_1997 +
      CV_HGC_RES_DAD_1997
  )
)
...
completed_data <- completed_data %>%
  mutate(fitted = model_plot$fitted.values, residuals = model_plot$residuals)
...
df <- data.frame(
  fitted = svy_model$fitted.values,
...
ggplot(
  ...
  coord_flip() +
  labs(
    title = "Pooled Regression Coefficients",
    x = "Predictor",
